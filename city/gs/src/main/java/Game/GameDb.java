			...
			.build(
					new CacheLoader<UUID, Player.Info>() {
						...
						public Player.Info load(UUID key) {
							...
							Transaction transaction = session.beginTransaction();
							org.hibernate.Query q = session.createQuery("SELECT new Game.Player$Info(id ,name, companyName, des, male) FROM Player where id = :x");
						...
						public Map<UUID, Player.Info> loadAll(Iterable<? extends UUID> keys) {
							StatelessSession session = sessionFactory.openStatelessSession();
							Transaction transaction = session.beginTransaction();
							org.hibernate.Query q = session.createQuery("SELECT new Game.Player$Info(id ,name, companyName, des, male) FROM Player where id in :x");
