...
//    }
    ...
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
        if(session.valid())
            City.instance().execute(()->session.logout(false)); // because logout will evict the player object, we must serialize the evict and all other logic which will get this object
    ...
    protected void channelRead0(ChannelHandlerContext ctx, Package msg) {
        if(!GameEventDispatcher.getInstance().process(msg, session)) {
            ...
            System.err.println("协议号不匹配，协议是:"+msg.opcode);
            ctx.close();
        }
    }
